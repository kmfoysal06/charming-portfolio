...
if (is_array($args) && array_key_exists("experiences", $args)):
 ...
 <tr class="flex flex-col simplecharm-basic-border simplecharm-basic-padding">
    ...
    <td>
        ...
        <input type="text" class="post-title" placeholder="<?php _e('Post Title','simplecharm-portfolio'); ?>" name="simplecharm_portfolio_plugin[experiences][<?php echo esc_attr($key); ?>][][post-title]" value="<?php echo (array_key_exists("post-title",$flattern_experience)) ? esc_attr($flattern_experience['post-title']) : '' ;?>" id="<?php echo esc_attr("experience-post-title-" . $key,'simplecharm-portfolio'); ?>" data-queue="<?php echo esc_attr($key); ?>">
    </td>
    <td class="responsibilities" id="repeatable-fieldset-three">
        ...
        <textarea name="simplecharm_portfolio_plugin[experiences][<?php echo esc_attr($key,'simplecharm-portfolio'); ?>][][responsibility]" id="<?php echo esc_attr("responsibilities-" . $key); ?>" cols="50" rows="5" class="responsibility" data-queue="<?php echo esc_attr($key); ?>"><?php echo esc_textarea((array_key_exists("responsibility",$flattern_experience)) ? $flattern_experience['responsibility'] : '') ;?></textarea>
    </td>
    <td>
        <label for="<?php echo esc_attr("start-date-" . $key); ?>"><?php _e("Start Date",'simplecharm-portfolio'); ?></label>
        <input type="date" class="start_date" name="simplecharm_portfolio_plugin[experiences][<?php echo esc_attr($key); ?>][][start_date]" placeholder="<?php _e("Start Date",'simplecharm-portfolio'); ?>" id="<?php echo esc_attr("start-date-" . $key); ?>" data-queue="<?php echo esc_attr($key); ?>" value="<?php echo (array_key_exists("start_date",$flattern_experience)) ? esc_attr($flattern_experience['start_date']) : '' ;?>">
    </td>
    <td>
        <label for="<?php echo esc_attr("end-date-" . $key); ?>"><?php _e("End Date",'simplecharm-portfolio'); ?></label>
        <input type="date" class="end_date" name="simplecharm_portfolio_plugin[experiences][<?php echo esc_attr($key); ?>][][end_date]" placeholder="<?php _e("End Date",'simplecharm-portfolio'); ?>" id="<?php echo esc_attr("end-date-" . $key); ?>" data-queue="<?php echo esc_attr($key); ?>" value="<?php echo (array_key_exists("end_date",$flattern_experience)) ? esc_attr($flattern_experience['end_date']) : '' ;?>" <?php disabled($working_now,'on');?>>
    </td>
    <td>
